#!/bin/bash
cd /home/ubuntu/tattant/server

# Load .env for DB credentials
export $(grep -v '^#' .env | xargs)

DB_NAME="${DB_NAME:-tattant}"
DB_USER="${DB_USER:-tattant_user}"

echo "=== Updating Burmese translations in database ==="
...
sudo -u postgres psql -d "$DB_NAME" -c "SELECT slug, name_my FROM categories ORDER BY display_order;"
sudo -u postgres psql -d "$DB_NAME" -c "SELECT name, LEFT(description_my, 60) as description_my_preview FROM businesses ORDER BY name;"

echo ""
echo "=== Done! Burmese translations updated successfully ==="
